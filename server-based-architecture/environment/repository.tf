...
module "base_project_ECR_celery_worker" {
  source   = "../common/ecr"
  ecr_name = terraform.workspace == "stg" ? "geacco_app_celery_worker_stg" : "geacco_app_celery_worker_prod"
}

module "base_project_ECR_celery_beat" {
  source   = "../common/ecr"
  ecr_name = terraform.workspace == "stg" ? "geacco_app_celery_beat_stg" : "geacco_app_celery_beat_prod"
}
...
resource "aws_iam_instance_profile" "celery_repository_intance_profile" {
  role = aws_iam_role.base_project_repository_role.name
}
